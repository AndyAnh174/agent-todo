#!/usr/bin/env python3
"""
Test script để kiểm tra chức năng gán tags tự động qua AI Agent
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/test_agent_auto_tag.log', encoding='utf-8'),
...
def create_todo_without_tags(token):
    ...
    logger.info("📝 Tạo todo không có tags...")
    ...
    if response.status_code == 201:
        ...
        logger.error(f"❌ Tạo todo thất bại: {response.status_code} - {response.text}")
        return None

def test_agent_auto_tag(token, todo_id):
    """Test Agent gán tags tự động"""
    logger.info(f"🤖 Test Agent gán tags tự động cho todo {todo_id}...")
    ...
    if response.status_code == 200:
        ...
        logger.error(f"❌ Agent chat thất bại: {response.status_code} - {response.text}")
        return None

def check_todo_tags(token, todo_id):
    """Kiểm tra tags của todo sau khi agent gán"""
    logger.info(f"🔍 Kiểm tra tags của todo {todo_id}...")
    ...
    if response.status_code == 200:
        ...
        logger.error(f"❌ Lấy todo thất bại: {response.status_code} - {response.text}")
        return None

def test_agent_create_todo_with_auto_tags(token):
    """Test Agent tạo todo mới với auto tags"""
    logger.info("🤖 Test Agent tạo todo mới với auto tags...")
...
def list_recent_todos(token):
    ...
    logger.info("📋 Lấy danh sách todos gần đây...")
    ...
    if response.status_code == 200:
        todos = response.json()
        logger.info(f"✅ Tìm thấy {len(todos)} todos:")
        ...
        logger.error(f"❌ Lấy danh sách todos thất bại: {response.status_code} - {response.text}")
        return []

def main():
    """Main test function"""
    logger.info("🚀 TEST AGENT AUTO TAG")
    logger.info("=" * 50)
